...
export class UserService {
    ...
    public async createSuperAdminUser() {
        ...
        if (!adminUser) {
          ...
          try {
            ...
            try {
              cognitoResp = await this.cognitoService.getUserFromCognito(adminEmailId);
      ...
      public async getUserByUserName(userName: string): Promise<User> {
        const user = await this.userRepo
          ...
          .addSelect([
            ...
            'client.name',
            'client.isActive',
            'client.clientBusinessType',
            'client.isEdiEnrollmentRequired',
          ])
  ...
  public async getUserByEmailId(emailId: string): Promise<User> {
    const user = await this.userRepo.findOneByCondRel(
      { emailId },
      {
        createdBy: true,
      },
    ...
    return user;
      ...
      public async singIn(emailId: string, password: string, isTPVAdminSignIn = false): Promise<string> {
